...
export const PrivateRoute = ({ component: Component, ...rest }) => {
  ...
  const errorMessage = (message) => {
    dispatch({
      ...
      payload: {
        alert: {
          open:true,status:"error",message
        },
      }})
  };
  const successMessage = (message) => {
    dispatch({
      ...
      payload: {
        alert: {
          open:true,status:"success",message
        },
      }})
  };
  return <Route {...rest} render={(props) => (
    cookie.load('EnToken') !== undefined
      ? <Component {...props} successMessage={successMessage} errorMessage={errorMessage}/>
      : <Redirect to='/signin' />
  )} />
}
...
export const cookieSave = (token) => cookie.save('EnToken', token, { path: '/' });
